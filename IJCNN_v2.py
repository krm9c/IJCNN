...
class Agent():
    ...
    def init_NN(self, R, Faults, lr, depth, Layers):
		...
		with tf.name_scope("Trainer"):
			...
			Latent_Loss =  -2* tf.reduce_sum(tf.log(self.Deep['FL_layer'+str(len(Layers)-1)]))

			# Latent_Loss = - tf.reduce_sum(self.Deep['FL_layer0']  \
			# * tf.log(1e-10 + self.Deep['FL_layer'+str(len(Layers)-1)]) + \
			# (1 - self.Deep['FL_layer0']) \
			# *tf.log( 1e-10 + 1 - self.Deep['FL_layer'+str(len(Layers)-1)] ) ,1)
			...
			#  Feature Learner
			self.Trainer["cost_Feature"] =  tf.reduce_mean( Latent_Loss+Error_Loss)
			tf.summary.scalar('cost_Feature', self.Trainer["cost_Feature"])
			self.Trainer['Optimizer_Feature']    =  tf.train.GradientDescentOptimizer(LearningRate)
			self.Trainer["TrainStep_Feature"] =  self.Trainer['Optimizer_Feature'].minimize(self.Trainer["cost_Feature"], global_step = global_step, var_list = Keys)
			...
			global_step = global_step, var_list = List)
...
def Training():
	...
	model = Agent()
	model = model.init_NN( X, Faults, 3e-4, 4, [X.shape[1], 1000, 1000, X.shape[1]])
	# Declare a saver
	for i in range(Train_Glob_Iterations):
		...
		for j in range(Train_Loc_Iterations):
			...
			for k in range(((P.shape[0])/Train_batch_size)):
				batch_xs  = P[prev:end, :]
				batch_ys  = Y[prev:end, :]
				summary, _  = model.sess.run([model.Summaries['merged'], model.Trainer['TrainStep_Feature']], feed_dict ={model.Deep['FL_layer0'] : batch_xs, model.classifier['Target']: batch_ys })
				if j % 10 == 0 :
